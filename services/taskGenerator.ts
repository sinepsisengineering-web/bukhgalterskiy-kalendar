...
// ... (вспомогательные функции isWeekend, isHoliday и т.д. остаются без изменений)
const RUSSIAN_HOLIDAYS = new Set<string>([ // <--- Добавили <string>
  // 2024
...
export const isTaskCompletable = (task: Task, currentDate: Date = new Date()): boolean => { if (!task.isPeriodLocked) { return true; } const today = new Date(currentDate); today.setHours(0, 0, 0, 0); const getQuarter = (date: Date) => Math.floor(date.getMonth() / 3); const taskDueDate = new Date(task.dueDate); const taskYear = taskDueDate.getFullYear(); const taskMonth = taskDueDate.getMonth(); const title = task.title.toLowerCase(); let periodStartDate: Date; if (title.includes('месяц')) { periodStartDate = new Date(taskYear, taskMonth, 1); } else if (title.includes('год')) { periodStartDate = new Date(taskYear, 0, 1); } else { const taskQuarter = getQuarter(taskDueDate); periodStartDate = new Date(taskYear, taskQuarter * 3, 1); } return today.getTime() >= periodStartDate.getTime(); };
// ... (конец блока без изменений)


// ИЗМЕНЕНИЕ 1: Функция теперь принимает LegalEntity вместо Client
const generateTasksForYears = (
  legalEntity: LegalEntity, // Принимаем LegalEntity
  years: number[],
  ...
  years.forEach(year => {
    taskGenerator(year).forEach(taskInfo => {
      // ИЗМЕНЕНИЕ 2: Используем ID юрлица для стабильности
      const stableSeriesId = `series-auto-${legalEntity.id}-${taskInfo.title.replace(/\s+/g, '-')}-${year}`;
      const rawDueDate = new Date(year, taskInfo.month, taskInfo.day);
      const dueDate = adjustDate(rawDueDate, rule);
      
      tasks.push({
        id: `${legalEntity.id}-${taskInfo.title.replace(/\s/g, '-')}-${year}-${taskInfo.month}-${taskInfo.day}`,
        // ИЗМЕНЕНИЕ 3: Заполняем новое поле legalEntityId
...
// ИЗМЕНЕНИЕ 4: Основная логика вынесена в функцию для одного юрлица
const generateTasksForLegalEntity = (legalEntity: LegalEntity): Task[] => {
  ...
  // ИЗМЕНЕНИЕ 5: Убран цикл, теперь используется одна система налогообложения
  const taxSystem = legalEntity.taxSystem;

  switch (taxSystem) {
    case TaxSystem.USN_DOHODY:
    case TaxSystem.USN_DOHODY_RASHODY:
      allTasks.push(...generateTasksForYears(legalEntity, yearsToGenerate, (year) => [
    ...
    case TaxSystem.PATENT:
      // Если основная система - Патент, то здесь можно добавить специфические задачи,
      // но основная логика патентов обрабатывается ниже в любом случае
      break;
  }
  
  // ИЗМЕНЕНИЕ 6: Логика патентов теперь работает с legalEntity.patents
      ...
      legalEntity.patents.forEach(patent => {
          ...
          yearsToGenerate.forEach(year => {
              ...
              const createPatentTask = (idSuffix: string, title: string, date: Date, rule: TaskDueDateRule) => ({
                ...
                legalEntityId: legalEntity.id, // Используем legalEntity.id
                title: `${title} «${patent.name}» за ${year}г.`,
                dueDate: adjustDate(date, rule),
                status: getTaskStatus(adjustDate(date, rule)),
                isAutomatic: true, dueDateRule: rule, repeat: RepeatFrequency.None, reminder: ReminderSetting.OneWeek, seriesId, isPeriodLocked: true,
              });
  ...
  // ИЗМЕНЕНИЕ 7: Логика по сотрудникам теперь работает с legalEntity.hasEmployees
...
// ИЗМЕНЕНИЕ 8: Новая главная функция, которая запускает генерацию для всех
