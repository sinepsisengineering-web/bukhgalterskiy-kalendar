...
// FIX: Define an interface for the confirmation modal props to allow React.ReactNode for the message, which fixes a type error where a JSX element was being assigned to a string.
...
// Base64 encoded simple notification sound (a short, pleasant "ping")
...
const App: React.FC = () => {
    ...
    const [tasks, setTasks] = useState<Task[]>(() => {
        try {
            ...
            return []; // Will be generated by useEffect
        } catch (error) {
    ...
    useEffect(() => {
        // Regenerate tasks whenever the list of active clients changes
        const autoGeneratedTasks = activeClients.flatMap(client => generateTasksForClient(client));
        const manualTasks = tasks.filter(task => !task.isAutomatic);
        const allTasks = [...autoGeneratedTasks, ...manualTasks];
        setTasks(updateTaskStatuses(allTasks));
    }, [activeClients]); // eslint-disable-line react-hooks/exhaustive-deps
    // Note: 'tasks' is intentionally omitted to prevent re-running on every task update
            ...
            tasks.forEach(task => {
                ...
                if (timeDiff > 0 && timeDiff <= reminderMillis) {
                    ...
                    new Notification('Напоминание о задаче', {
                        ...
                        // icon: '/assets/icon.png' // Would need an icon file
                    });
    ...
    const handleSaveClient = (clientData: Client) => {
        setClients(prev => {
            const existing = prev.find(c => c.id === clientData.id);
            if (existing) {
    ...
    const handleToggleComplete = useCallback((taskId: string, currentStatus: TaskStatus) => {
        ...
        setTasks(prevTasks => {
            ...
            // After changing status, re-check if the detail modal should close
            ...
            if (updatedDetailTasks.every(t => t.status === TaskStatus.Completed)) {
                setTimeout(() => setIsTaskDetailModalOpen(false), 500); // Close after a short delay
            }
